...
--- SQL представления (создаётся приложением, см. data/clickhouse.py) ----------
Представление поднимает сам дашборд на старте (`clickhouse.ensure_schema` →
`_ensure_analytics`): если его нет — создаёт и считает всю историю, если оно
отстало от swaps/transactions — досыпает недостающие блоки. Ниже — то же тело,
что уходит в БД (единственный источник правды — константы в data/clickhouse.py).
